...
An event-driven, asyncronous platform
...
Same as above, but set up an event listener for request:
    var http = require('http');
...
To crate a chat app, we would use __websockets__: lightweight bidirectional protocol rather than HTTP. HTTP could be used to poll the server, but it opens a new TCP/IP connection every time (not effecient for chat apps)

## Syncronous Functions

Node’s API is mostly asynchronous functions that never block the event loop, so why bother including synchronous versions of these filesystem functions? The answer is that Node’s own `require()` function is synchronous, and it’s implemented using the `fs` module functions, so synchronous counterparts were necessary. Nevertheless, in Node, __synchronous functions should only be used during startup, or when your module is initially loaded, and neverafter that__.

## NPM

Quickstart a project, build package.json:
    $ npm init
...
Update packages - all package.json dependencies will be updated
    # npm update --save
...
Helper functions, frameworks, etc
attach to 'exports', assign to 'module.exports' (takes precedence)
*require() modules at the top of the application because it is syncronous and apps would take a performance hit if it's called during i/o*
...
GET is a safe method; no modifications are made (ie search inquiry)
POST is unsafe; it updates or modifies
